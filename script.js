...
function addQuestion(sub, body) {
    ...
    let lastFavoritedIndex = -1;
    children.forEach((child, index) => {
        const notFav = child.querySelector('.bstar').src;
        if (notFav.endsWith('colorStar.png')) {
            lastFavoritedIndex = index;
        }
    });
    if (lastFavoritedIndex !== -1) {
        second.insertBefore(newdiv, children[lastFavoritedIndex + 1]);
    } else {
...
function createResponseDOM(name, comment, id, like = 0, dislike = 0) {
    ...
    const likeCounter = newdiv.querySelector('.likeCounter');
    const dislikeCounter = newdiv.querySelector('.dislikeCounter');
